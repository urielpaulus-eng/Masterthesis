...
def  define_nodes(tcc_element_length,cross_section_height,cross_section_width,num_mesh_elements_x,num_mesh_elements_y,num_mesh_elements_z):  # def = Definiere Funktion mit Namen und Parametern
    ...
    n_nodes_x = num_mesh_elements_x+1  # Rechne Anzahl Knoten: Elemente + 1
    if num_mesh_elements_x == 0:  # if = Wenn Bedingung erfüllt ist (== bedeutet "ist gleich")
        ...
        l_element_x = tcc_element_length/num_mesh_elements_x  # / = Division
    ...
    n_nodes_y = num_mesh_elements_y + 1            # Anzahl Knoten in y-Richtung

    if num_mesh_elements_y == 0:
        b_element_y = 0
    else:
        b_element_y = cross_section_height / num_mesh_elements_y      # Elementhöhe (b über n_el_y)

    if num_mesh_elements_y % 2 == 0:
    ...
    n_nodes_z = num_mesh_elements_z+1  # Rechne Anzahl Knoten
    if num_mesh_elements_z == 0:  # Wenn keine Elemente in z-Richtung
        h_element_z = 0  # Elementhöhe = 0
    else:  # Sonst
        h_element_z = cross_section_width/num_mesh_elements_z  # Teile Höhe durch Anzahl Elemente
    if num_mesh_elements_z % 2 == 0:  # Wenn gerade Anzahl
    ...
    elif num_mesh_elements_z % 2 != 0:  # Wenn ungerade Anzahl; != bedeutet ungleich; → wenn gerade Zahl ist rest ==0 und wenn ungerade Zahl rest!=0
...
def define_quadrilateralN4(num_mesh_elements_x,num_mesh_elements_y,num_mesh_elements_z):  # Definiere Funktion für Viereckelemente
    
    if num_mesh_elements_z > 0:  # > bedeutet "größer als"
    ...
    if num_mesh_elements_y % 2 == 0:  # Wenn gerade Anzahl Elemente in y-Richtung
        for k in range(num_mesh_elements_x):  # Schleife über alle Elemente in x-Richtung
            element_1 = num_mesh_elements_y*(k+1)-num_mesh_elements_y/2+1  # Berechne Elementnummer → Element 1 ist für Elemente über der Mitte, also y>=250
            ...
            element_2 = num_mesh_elements_y*(k+1)-num_mesh_elements_y/2  # Zweites Element (unter der Mitte)
            ...
            for i in range(int(num_mesh_elements_y/2-1)):  # Schleife für restliche Elemente in y-Richtung
