...
from config_hbv_3d import GEOMETRY, MATERIAL_TIMBER, MATERIAL_CONCRETE, MATERIAL_KERVE, LOADCASE

def get_y_regions_from_geometry(GEOMETRY):
    b = GEOMETRY["b"]
    b_timber = GEOMETRY["b_timber"]
    b_concrete = GEOMETRY["b_concrete"]

    # y = 0 ist in der Mitte (so arbeitet deine define_nodes-Funktion)
    y_bottom = -b / 2.0
    y_top    = +b / 2.0

    y_timber_top      = y_bottom + b_timber
    y_concrete_bottom = y_timber_top
    y_concrete_top    = y_top

    return {
        "y_bottom": y_bottom,
        "y_timber_top": y_timber_top,
        "y_concrete_bottom": y_concrete_bottom,
        "y_top": y_top,
    }
...
if __name__ == "__main__":
    print("GEOMETRY aus config_hbv_3d.py:")
    print(GEOMETRY)

    # y-Bereiche einmal ausgeben
    y_info = get_y_regions_from_geometry(GEOMETRY)
    print("\ny-Bereiche nach GEOMETRY (in mm):")
    print(y_info)
    ...
    print("\nMin/Max y:")
    print("y_min =", df_nodes["y"].min())
    print("y_max =", df_nodes["y"].max())
    ...
    df_elements.insert(1, "Propertie", 0)

    # Wenn du wieder Dateien schreiben willst:
    # df_nodes.to_csv("nodes.csv", sep="\t", index=False)
    # df_nodes_xyz.to_csv("nodes_xyz.csv", sep="\t", index=False)
    # df_nodes_number.to_csv("nodes_number.csv", sep="\t", index=False)
    # df_elements.to_csv("elements.csv", sep="\t", index=False)
    # df_elements_number.to_csv("elements_number.csv", sep="\t", index=False)
